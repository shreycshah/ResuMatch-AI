"""
LLM API clients for Anthropic (Claude Haiku 4.5) and OpenAI (GPT-4.1 mini).
...
def call_anthropic(resume: ResumeJSON, jd: JobDescription) -> dict:
    """Call Claude Haiku 4.5 with prompt caching on the system prompt."""
    try:
        import anthropic
    except ImportError:
    ...
    for attempt in range(1 + MAX_RETRIES):
        ...
        if stop == "max_tokens":
            if attempt < MAX_RETRIES:
                print(f"  ⚠ Response truncated (hit {MAX_OUTPUT_TOKENS} tokens), retrying with shorter prompt...")
